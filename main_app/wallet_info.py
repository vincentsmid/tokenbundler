        ...
        for key, value in item_data.items():
            if key == 'metadata' and value:
                try:
                    ...
                    self.metadata_description = metadata.get('description', 'No description provided.')
                except json.JSONDecodeError:
                    self.metadata_name = 'Metadata parsing error'
                    self.metadata_description = 'Metadata parsing error'
            elif key == 'media' and value:
                self.media_low = value.get('media_collection', {}).get('low', {}).get('url', 'No low-resolution media link.')
            else:
                setattr(self, key, value)

async def get_wallet_tokens(address: str, chain: str):
  params_tokens_request = {
    "chain": chain,
    "address": address
  }
  ...
  # Debug
  print(json.dumps(result_tokens['result'], indent=4))
...
async def get_wallet_nft(address: str, chain: str):
  ...
  # Debug
  print(json.dumps(result_nft['result'], indent=4))
  ...
  for item in result_items:
    response_nft = {
      ...
      "description": getattr(item, 'metadata_description', 'No description provided.'),
